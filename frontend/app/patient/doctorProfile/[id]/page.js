...
export default function DoctorProfilePage() {
  ...
  const handleBookingConfirm = async () => {
    ...
    try {
      ...
      setConfirmation({
        ...
        doctor,
        joinLink:
          consultType === "video" ? "https://video.consult/join/abc123" : null,
      });
  ...
  if (loading)
    return <div className="p-6 text-center">Loading doctor profile...</div>;
    ...
    <div className="max-w-3xl mx-auto p-6">
        ...
        <CardContent className="flex flex-col sm:flex-row gap-6 items-center p-6">
          <Avatar className="w-24 h-24">
            <AvatarImage
              src={
                doctor?.avatarUrl
                  ? `http://localhost:5000${doctor.avatarUrl}`
                  : "/default-avatar.png"
              }
            />
      ...
      {/* <Card className="mb-8 rounded-3xl bg-white/90 shadow border-0">
        <CardContent className="p-8">
          <div className="font-bold text-lg text-blue-900 mb-2 border-l-4 border-blue-200 pl-2">
            About
          </div>
          <Textarea
            value={doctor?.bio}
      ...
      <Card className="mb-8 rounded-3xl bg-white/90 shadow border-0">
        <CardContent className="p-8">
          <div className="font-bold text-lg text-blue-900 mb-4 border-l-4 border-blue-200 pl-2">
            Available Slots
          </div>
          <SlotPicker
            doctorId={doctor?._id}
      ...
      <Card className="mb-8 rounded-3xl shadow bg-white/95 border-0">
        <CardContent className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-8">
          <div>
            <div className="font-bold text-lg text-blue-900 mb-3 border-l-4 border-blue-200 pl-2">
              Consultation Mode
            </div>
      ...
      <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-blue-800 font-extrabold text-2xl mb-4">
              Confirm Appointment
            </DialogTitle>
          </DialogHeader>
          <div className="mb-2">
            Doctor:{" "}
            <span className="font-semibold">{doctor?.fullName || "D"}</span>
          </div>
          <div className="mb-2">
            Slot:{" "}
          ...
          <div className="mb-2">
            Duration: <span className="font-semibold">{duration} min</span>
          </div>
          <div className="mb-2">
            Fee: <span className="font-semibold">₹{doctor?.fee || "200"}</span>
          </div>
          <div className="mb-2">
            Patient: <span className="font-semibold">{patientName}</span>
          </div>
